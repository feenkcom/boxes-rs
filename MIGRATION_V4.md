...
- `take_value()` exists only on `OwnedPtr<T>`.
...
- call `take_value()` only on `OwnedPtr<T>`
...
| `ptr.take_value()`                       | `OwnedPtr<T>` arg        | `take_value()`                         |
...
pub fn foo_run(foo: *mut ValueBox<Foo>) {
    foo.take_value().map(|foo| foo.run()).log();
}
```
...
- `ptr.take_value()` -> `ptr.take_value()` where `ptr: OwnedPtr<T>`
...
1. If the function calls `take_value`, `release`, or otherwise destroys the box, the parameter becomes `OwnedPtr<T>`.
...
- calling `take_value()` through a borrowed pointer type
...
- `take_value()` on `OwnedPtr::null()` still returns `NullPointer(...)`
...
5. Search for `.release()`, `.take_value()`, `*mut ValueBox<`, `ValueBox::new`, and deprecated helper names.
